...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="1.3",
        help="GFPGAN model version. Option: 1 | 1.2 | 1.3. Default: 1.3",
    )
    parser.add_argument(
        "-s",
        "--upscale",
    ...
    parser.add_argument(
        "--aligned", action="store_true", help="Input are aligned faces"
    )
    parser.add_argument(
        "--ext",
        type=str,
    ...
    elif args.version == "1024":  # this is my trained version
        arch = "1024"
        channel_multiplier = 2
        model_name = "GFPGANx1024"
        url = ""
    ...
    for img_path in img_list:
        ...
        print(f"  Image read. Size: {input_img.shape}")
        ...
        print(f"  Enhancement complete. Found {len(cropped_faces)} face(s)")

        # save faces
        for idx, (cropped_face, restored_face) in enumerate(
            zip(cropped_faces, restored_faces)
        ):
            print(f"  Processing face {idx+1}/{len(cropped_faces)}")
            ...
            # print(f"    Saved cropped face: {save_crop_path}")
            ...
            # print(f"    Saved restored face: {save_restore_path}")
        ...
        if restored_img is not None:
            ...
            # print(f"  Saved restored image: {save_restore_path}")
